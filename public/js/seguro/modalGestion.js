document.addEventListener('DOMContentLoaded', () => {
  const modalGestion = document.getElementById('modalGestionSeguro');
  if (!modalGestion) return;

  const form = document.getElementById('formGestionSeguro');
  const errorContainer = document.getElementById('error-container-gestion-seguro');
  const idInput = document.getElementById('gestion_seguro_id');
  const nombreInput = document.getElementById('gestion_nombre_seguro');
  const btnActivar = document.getElementById('btn-activar-seguro');
  const btnDesactivar = document.getElementById('btn-desactivar-seguro');
  ...
  modalGestion.addEventListener('show.bs.modal', async (event) => {
    ...
    try {
      const response = await fetch(`/api/seguros/${seguroId}`);
      if (!response.ok) {
        throw new Error('No se pudo obtener la información del seguro medico.');
      }
      const seguro = await response.json();

      // Poblar el formulario
      idInput.value = seguro.id;
      nombreInput.value = seguro.nombre;
      ...
      console.error('Error al cargar datos de seguro medico:', error);
      ...
      bootstrapModal.hide();
      mostrarAlerta('Error al cargar los datos de la seguro.', 'danger');
    }
  });
  ...
  const actualizarFilaTabla = (id, data) => {
    const fila = document.getElementById(`seguro-row-${id}`);
    if (!fila) return;

    if (data.nombre) {
      const celdaNombre = fila.querySelector('.nombre-seguro');
      if (celdaNombre) celdaNombre.textContent = data.nombre;
    }

    if (typeof data.activo === 'boolean') {
      const celdaEstado = fila.querySelector('.estado-seguro');
      if (celdaEstado) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      mostrarAlerta('Seguro Medico actualizada con éxito.', 'success');
      // Actualizamos la tabla dinámicamente
      actualizarFilaTabla(seguroId, { nombre: data.nombre });
    } catch (error) {
  ...
  const cambiarEstado = async (nuevoEstado, botonPresionado) => {
    ...
    try {
      const response = await fetch(`/api/seguros/${seguroId}/activo`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      mostrarAlerta(`Seguro Medico ${verbo === 'activar' ? 'activada' : 'desactivada'} correctamente.`, 'success');

      // Actualizar la tabla dinámicamente
      actualizarFilaTabla(seguroId, { activo: nuevoEstado });
